...
type clusterOffsets struct {
	...
	// broker list PartitionsLeader  id -> broker ip
	brokerList map[int32]string

	partitionsLeader map[int32]string
...
func (module *InMemoryStorage) Start() error {
	...
	for cluster := range viper.GetStringMap("cluster") {
		module.
			...
			brokerList:       make(map[int32]string),
			partitionsLeader: make(map[int32]string),
			brokerLock:       &sync.RWMutex{},
			consumerLock:     &sync.RWMutex{},
		}
	}
...
func (module *InMemoryStorage) requestWorker(workerNum int, requestChannel chan *protocol.StorageRequest) {
	...
	var requestTypeMap = map[protocol.StorageRequestConstant]func(*protocol.StorageRequest, *zap.Logger){
		...
		protocol.StorageFetchPartitionsLeader:  module.fetchPartitionsLeader,
...
func (module *InMemoryStorage) mainLoop() {
	...
	for r := range module.requestChannel {
		...
		case protocol.StorageSetBrokerOffset, protocol.StorageSetDeleteTopic, protocol.StorageFetchClusters, protocol.StorageFetchConsumers, protocol.StorageFetchTopics, protocol.StorageFetchTopic, protocol.StorageFetchConsumersForTopic, protocol.StorageFetchPartitionsLeader:
			// Send to any worker
...
func (module *InMemoryStorage) addBrokerOffset(request *protocol.StorageRequest, requestLogger *zap.Logger) {
	...
	for k, v := range request.PartitionsLeader {
		clusterMap.partitionsLeader[k] = v
	}

	for k, v := range request.BrokerList {
		clusterMap.brokerList[k] = v
	}
...
func (module *InMemoryStorage) fetchPartitionsLeader(request *protocol.StorageRequest, requestLogger *zap.Logger) {
	defer close(request.Reply)
	...
	PartitionsLeader := clusterMap.partitionsLeader
	clusterMap.brokerLock.RUnlock()

	requestLogger.Debug("ok")
	request.Reply <- PartitionsLeader
}
