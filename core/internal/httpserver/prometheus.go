...
var (
	...
	consumerPartitionCurrentOffset = promauto.NewGaugeVec(
		...
		[]string{"cluster", "consumer_group", "topic", "partition", "owner", "partition_Leader"},
	)
	...
	return http.HandlerFunc(func(resp http.ResponseWriter, req *http.Request) {
		for _, cluster := range listClusters(hc.App) {
			for _, consumer := range listConsumers(hc.App, cluster) {
				...
				partitionsLeader := listPartitionsLeader(hc.App, cluster)

				for _, partition := range consumerStatus.Partitions {
					if partition.Complete < 1.0 {
						continue
					}
					leader, _ := partitionsLeader[partition.Partition]
...
func listPartitionsLeader(app *protocol.ApplicationContext, cluster string) map[int32]string {
	request := &protocol.StorageRequest{
		RequestType: protocol.StorageFetchPartitionsLeader,
		Cluster:     cluster,
		Reply:       make(chan interface{}),
	}
	app.StorageChannel <- request
	...
	return response.(map[int32]string)
}
