	...
	if module.fetchMetadata {
		...
		for _, topic := range topicList {
			...
			for _, partitionID := range partitions {
				if sc, err := client.Leader(topic, partitionID); err != nil {
					...
					partitionsLeader[partitionID] = sc.Addr()
					brokerList[sc.ID()] = sc.Addr()
				}
			}
		}
		...
		module.PartitionsLeader = partitionsLeader
		module.BrokerList = brokerList
	}
}
		...
		for topic, partitions := range response.Blocks {
			for partition, offsetResponse := range partitions {
				...
				offset := &protocol.StorageRequest{
					...
					PartitionLeader:     module.PartitionsLeader[partition],
					PartitionsLeader:    module.PartitionsLeader,
					BrokerList:          module.BrokerList,
				}
